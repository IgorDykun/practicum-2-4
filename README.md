...
.NET SDK — переконайтеся, що у вас встановлено .NET 8 SDK або .NET 9 SDK (перевірити можна командою dotnet --version).

Docker Desktop — обов'язково має бути запущений. Оркестратор .NET Aspire автоматично підніме та налаштує необхідні контейнери для баз даних MS SQL Server.

Papercut SMTP — встановіть та запустіть цю утиліту локально. Вона автоматично перехоплюватиме транзакційні сервісні листи на порту localhost:25.

Кроки для запуску проекту через CLI:
Відкрийте термінал (PowerShell, CMD або Bash) у кореневій папці проекту (там, де лежить файл .slnx).

Перейдіть у папку оркестратора Aspire:
...
Bash
dotnet run
У консолі з'явиться посилання на .NET Aspire Dashboard (наприклад, http://localhost:15111). Відкрийте його у браузері.

На панелі керування Aspire знайдіть сервіс webendpoint і перейдіть за його посиланням, додавши в кінець /swagger для доступу до інтерактивної документації API.
...
Зареєструйте нового користувача з роллю адміністратора та увійдіть в систему, щоб отримати сесійні Cookies/JWT-токен. Це перевіряє захист системи (ЛР 1, 2, 5).

 Створення продукту (POST /products)

Додайте новий товар (наприклад, "Ноутбук") з базовою ціною. Переконайтеся, що рольовий захист працює, і звичайний користувач без прав Admin отримає помилку 403 Forbidden (ЛР 3, ЛР 5). Скопіюйте згенерований id продукту.

 Створення покупця (POST /customers)

Створіть профіль клієнта. Модуль автоматично згенерує безпечний тимчасовий пароль (перші 12 символів GUID), створить акаунт в Identity, а інфраструктурний модуль Email надішле вітальний лист. Перевірте його появу в утиліті Papercut (ЛР 4, ЛР 6). Скопіюйте customerId.

 Створення замовлення (POST /orders)

Надішліть запит на створення замовлення, передавши customerId та productId. Ціну передавати не потрібно — система автоматично виконає кросмодульний запит GetProductPriceQuery до каталогу товарів для забезпечення фінансової безпеки й запобігання маніпуляціям з боку клієнта (ЛР 6).

 Підтвердження та Інжест (POST /orders/{id}/confirm)

Переведіть створене замовлення в статус Confirmed. Це активує публікацію внутрішньопроцесної події OrderCreatedEvent через Mediator. Аналітичний обробник модуля Reporting перехопить подію та ідемпотентно (уникаючи дублікатів через унікальний композитний індекс) рознесе дані по таблицях фактів та вимірів в окрему базу даних reportingdb (ЛР 7).

 Генерація аналітичного звіту (GET /reports/orders)
